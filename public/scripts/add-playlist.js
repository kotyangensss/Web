function addPlaylist() {
  ...
  btn.addEventListener('click', () => {
    ...
    if (isAdd !== null) {
      while (btn.nextSibling !== isAdd) btn.nextSibling.remove();
      ...
      add.addEventListener('click', () => {
        if (playlist_name.value !== '') {
          ...
          let new_playlist = JSON.parse(
            localStorage.getItem('playlists'),
          ).playlists;
          new_playlist.push(id);
          localStorage.setItem(
            'playlists',
            JSON.stringify({ playlists: new_playlist }),
          );

          while (btn.nextSibling !== add) btn.nextSibling.remove();
          btn.nextSibling.remove();

          let list = document.getElementsByClassName('playlists__list')[0];
          if (list !== undefined) list.remove();
          loadPlaylists();
        }
      });
    }
